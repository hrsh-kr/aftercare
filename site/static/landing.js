...
(async function () {
  ...
  try {
    ...
    // on the static (Vercel) copy the probe is a recording (replay-shim.js sets `recorded`): never call it live
    const tag = h.recorded ? "recorded" : "live";
    set("runtime", h.runtime === "lambda", h.recorded ? "● recorded from a Lambda on SAM Local" : h.runtime === "lambda" ? "● this page was served by a Lambda" : "○ not running in Lambda");
    set("ollama", c.ollama.ok, c.ollama.ok ? `● ${tag} · ${c.ollama.ms} ms · ${c.ollama.models.includes("gemma2:9b") ? "gemma2:9b" : c.ollama.models[0]}` : "○ offline: start ollama");
    set("opensearch", c.opensearch.ok, c.opensearch.ok ? `● ${tag} · v${c.opensearch.version} · ${c.opensearch.documents} sections indexed` : "○ OpenSearch unreachable");
    set("cedar", c.cedar.ok, c.cedar.ok ? `● ${tag} · policies validated · ${c.cedar.policy_sha}` : "○ Cedar CLI missing");
    set("storage", c.dynamodb.ok, c.dynamodb.ok ? `● ${tag} · DynamoDB Local · table ${c.dynamodb.table}` : "○ DynamoDB unreachable");
