...
4. A washing-machine drum-noise complaint, in the customer's own words, shown on the two-pane view — the customer's WhatsApp and AquaSpin's WhatsApp Business inbox side by side, the same conversation mirrored on both. Show the agent give one real step (check it's level, spread the load), wait for a reply, and close it out — resolved, no ticket, no human involved.
5. An AC cooling complaint that isn't fixed by the first step — show the second, different step offered, and this time it's still broken: show it escalate with exactly what was already tried, not a guess dressed up as a third attempt. Click the "Open dashboard" button that appears on ArcticAir's side right here — the deliberate hand-off from live chat to the dashboard.
