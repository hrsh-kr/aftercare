...
| 2:30–2:50 | Terminal: `scripts/dev.sh` running, then `run_scenarios.py` all PASS. Stop OpenSearch to show the 503 naming it. Demo pill: "Served by AWS Lambda (SAM Local) · DynamoDB · OpenSearch · Cedar · Ollama" | "The whole site is one Lambda behind API Gateway in SAM Local, on DynamoDB Local running on Corretto, with Powertools routing, logging and idempotency. No fallbacks: stop OpenSearch and it says so. Five scenarios, five endings." |
