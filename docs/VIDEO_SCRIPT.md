...
| 1:05–1:30 | Dashboard section: scroll the complaints, click a row to show the chat, point at the red/yellow/green dots and the AI analytics bar (Details); click **Try ArcticAir's (refused)** | "This is what the brand sees: every complaint and its status, the chat behind it, and analytics computed by OpenSearch aggregations. Cedar decides who may do what, and it refused AquaSpin's manager on ArcticAir's dashboard, naming the policy." |
| 1:30–2:30 | Conversations: Arjun (bangs → fixed), Neha (warranty), Rohan (Hi → smell → pick FC-700 → recurring), Divya (not in the manual), Meera (person → reply → agent refused → manager resolves). Point at the trace tags | "Press Send and you replay what happened. One step from the manual, fixed means no ticket. Warranty is arithmetic on the purchase date, no model. Rohan's problem was back within 90 days, found by an OpenSearch time-window query, so it doesn't repeat the fix. If it isn't in the manual it won't guess. A person replies in the same chat, and Cedar lets an agent reply but only a manager resolve." |
