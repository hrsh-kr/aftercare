...
A static host can't run the stack, so the deployed site is pre-rendered from the *same* Jinja templates the Lambda serves. `scripts/build_site.py` writes three pages into `public/` (the folder Vercel serves, per `vercel.json`): the landing page, the live-demo page, and one dashboard snapshot. Push to GitHub and Vercel redeploys; `tests/test_site_fresh.py` fails if a template changed without re-running the build, so the deployed pages cannot drift from the source.

The live-demo page (`/demo`) mirrors the real sandbox (order file, customers, what Aftercare did, inbox with a person replying, analytics) and replays `public/static/recording.json`, which `scripts/record_playback.py` captured from the running stack through the real Meta-shaped webhook and the staff endpoints (nothing hand-written except the persona blurbs): eight customers, each reply's trace, the tickets, the dashboard's API responses. The dashboard is the real `dashboard.js` over those recorded responses, framed read-only and sized to its content. `public/static/replay-shim.js` answers the few `/api` calls the landing page and dashboard make from the recording, so on the static site the status chips say "recorded", never "live". The real, free-form version (`/sandbox`, `/dashboard`) runs only locally.
