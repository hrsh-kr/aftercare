...
## Sandbox pass (registry, channel, model quality)
- **OpenSearch synonyms are the right tool, but the analyzer needs a custom definition**: `english` has no synonym hook, so we defined `aftercare_english` (standard tokenizer, `synonym` filter with `lenient: true`, stop, stemmer). The catch is the same knowledge must live in your application code too (our "is this even in the manual?" gate), or the two layers disagree; we generate both from one Python list. Changing the analyzer means a new index (v4).
- **DynamoDB single-table with three overloaded GSI namespaces** (ticket id, serial, phone) worked cleanly with prefixes; the friction is boto3 returning numbers as `Decimal`, which is not JSON-serializable, so a `_native()` pass is needed on every read path. `Scan` is only used for the demo reset, and paginates at 1 MB.
