...
- [ ] Record the 3-minute demo video (script: `VIDEO_SCRIPT.md`)
...
## Phase 7.20: Story, colour, spreadsheet, architecture screen, final audit (done, 2026-09-20)
- **One website.** The deployed site is the repo's `public/`, pre-rendered from the Lambda's own templates (`scripts/build_site.py`); the Live demo page replays real responses recorded from the running stack (`scripts/record_playback.py`); `tests/test_site_fresh.py` fails on stale pages or leaks. The separate test site and the old scripted demo are gone.
- **Live demo redesign (several rounds of feedback).** It now mirrors the real sandbox: an order file shown as a live spreadsheet (rows scan; green rows are registered under their brand, red rows show the reason and the offending cell), eight customer cards, a phone with click-to-send chips, and three brand tabs (What Aftercare did, Inbox, Analytics). The ledger is one running list across messages until the case is fixed, ticketed or with a person. Look: light page, greys that darken down the page in full-width bands, one dark band, and whole-card gradients (after the Apple AirPods page).
- **Architecture screen on the landing page.** One blueprint-styled screen replaces the old "Under the hood" chapter: the message path (customer, API Gateway, Lambda, services) and five cards (what each AWS tool does, why, the trade-off), each with its live/recorded status chip, and a strip on what is never left to the model and the managed-service deploy path.
- **Final audit.** Lint clean (`ruff.toml` selects real-bug rules: syntax, pyflakes, bugbear); all `tests/run_all.sh` suites pass; `scripts/run_sandbox_scenarios.py` 25/25 against the real model; `scripts/eval_routing.py` 41/41; no secrets in the tracked tree or history; private working files (internal notes, form drafts, archive) git-ignored.
- **Docs for submission.** `BLOG.md` (the real story: the Ship It plan, the account-verification block, GroundTruth to Aftercare, the bugs), `VIDEO_SCRIPT.md` (3 minutes, three parts, Hormozi-style structure), `AWS_FEEDBACK_LOG.md` (onboarding frictions added).
- **Lessons that repeated:** check the rendered page after every edit (a regex deleted the landing phone once); remove fallbacks so failures are visible; never let a model do arithmetic; record real behaviour instead of simulating it, and label it a recording.
